...
```{r variant4-stable, test = list(result_v4-stable = "result")}
lyt <- basic_table(show_colcounts = TRUE) %>%
  split_cols_by("ARM", ref_group = "A: Drug X") %>%
  analyze_vars(
    vars = "is_event",
...
```{r variant4-devel, test = list(result_v4-devel = "result")}
lyt <- basic_table(show_colcounts = TRUE) %>%
  split_cols_by("ARM", ref_group = "A: Drug X") %>%
  analyze_vars(
    vars = "is_event",
...
app <- init(
  ...
  modules = modules(
    tm_t_tte(
      ...
      time_points = teal.transform::choices_selected(c(6, 8), 6),
