...
adab_pb_ada <- df_explicit_na(adab) %>%
  filter(ADABLPFL == "Y") %>%
  select(STUDYID, USUBJID, ARM, ACTARM, ADABLPFL) %>%
  mutate(ADABLPFL = ADABLPFL == "Y") %>%
  distinct()
...
adab_pb <- left_join(adab_pb_ada, adab_pb_adap, by = mergecol) %>%
  ...
  var_relabel(
    ADABLPFL = "Post-baseline evaluable patients for ADA",
...
lyt_pb <- basic_table(show_colcounts = TRUE) %>%
  ...
  count_patients_with_flags(
    "USUBJID",
    flag_variables = formatters::var_labels(adab_pb[, c("ADABLPFL", "ADAPFL")]),
    .stats = "count",
